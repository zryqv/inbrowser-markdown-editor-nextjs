...
function Toggle({ isLight, setIsLight }: ToggleProps) {
  return (
    <div className="flex w-fit items-center justify-center  py-6">
      <svg
        width="17"
        height="16"
        xmlns="http://www.w3.org/2000/svg"
        className={`${isLight ? "fill-600" : "fill-100"}`}
      >
      ...
      <button
        ...
        className={`flex h-6 w-12 items-center rounded-full bg-600 p-[6px] ${
          isLight ? "justify-end" : "justify-start"
        } mx-[0.7rem]`}
      >
        <div className="h-3 w-3 rounded-full bg-100"></div>
      </button>
      <svg
        width="18"
        height="18"
        xmlns="http://www.w3.org/2000/svg"
        className={`${isLight ? "fill-100" : "fill-600"}`}
      >
