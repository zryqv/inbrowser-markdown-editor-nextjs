...
import PastDocument from "./shared/PastDoc";
import Toggle from "./shared/Toggle";
import type { ToggleProps } from "./shared/Toggle";
...
type SidebarProps = {
  ...
  setDoc: Dispatch<SetStateAction<ClientDocument>>;
} & ToggleProps;
function Sidebar({
  currentDoc,
  content,
  ...
  return (
    <div className="flex h-screen w-[15.625rem] flex-col items-start justify-between bg-900 p-6">
      <div className="">
        ...
        <div className="py-7 text-hs uppercase text-500 tablet:pt-0">
          my documents
        </div>
        <Button
          className="w-[12.62rem]"
          onClick={() => {
            const newdoc: ClientDocument = {
              notinDb: true,
              title: getNewTitle(
                docs,
                docs.find((doc) => doc.title.includes("untitled-document"))
            ...
            setDocs([
              newdoc,
              ...docs.map((oldDoc) => {
                if (oldDoc.title === currentDoc.title)
                  return {
                    notinDb: true,
      ...
      <div className="">
        <Toggle isLight={isLight} setIsLight={setIsLight} />
      </div>
    </div>
  );
}
...
function getNewTitle(docs: ClientDocument[], oldTitle: string): string {
  const exists = docs.find((doc) => doc.title === oldTitle);
  if (!exists) return oldTitle;
  const title = exists.title;
  const words = title.split("-");
  if (isNaN(+(words[words.length - 1] as string))) return `${oldTitle}-1`;

  return `${oldTitle.slice(
    0,
    oldTitle.length - (words[words.length - 1] as string).length
  )}${+(words[words.length - 1] as string) + 1}`;
}
